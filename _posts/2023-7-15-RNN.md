...
subtitle: Almost everything to get started with RNN
cover-img: /assets/img/AdobeStock_292765771-1080x675.jpeg
thumbnail-img: /assets/img/RNN.png
...
Proper initialisation of weights seems to have an impact on training results there has been lot of research in this area. It turns out that the best initialisation depends on the activation function (tanh in our case) and one [recommended](http://jmlr.org/proceedings/papers/v9/glorot10a/glorot10a.pdf) approach is to initialise the weights randomly in the interval from`[ -1/sqrt(n), 1/sqrt(n)]`where `n` is the number of incoming connections from the previous layer.
