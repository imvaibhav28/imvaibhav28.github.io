...
is called **word2Vec**. Embeddings mathematically signify mapping of one space to another not ensuring the retention of meaning of each space.  I am going to implement this from scratch. < hopefully >

---

### Word2Vec 
Let's get into details of what word2Vec embedding are . How are these made. Time to dive into some internet resources.

Alright here is some fundamental mathematics behind construction of word2Vec.
Things I got to know are:

+ word2Vec spits out **static** embeddings (vectors) for each words in vocab
 < What if same word occurs twice in different context in same doc?>
...
task; instead we’ll take the learned classifier weights as the word embeddings. [Source](https://web.stanford.edu/~jurafsky/slp3/).

So basically let's assume a sentence W -> " w1 w2 w3 w4 w5 w6" having 6 words. Steps involved in a skip gram models are:


- For each word w in W, < lets say w3 >, calculate the probability of w2 and w4 occuring in running sequence of w3
- To calculate this probability, we use a binary classification method of logistic regression.

Alright, but does it make sense? Well, I guess the higher probability of a words occuring in a sequence gives more
importance to the **context** of a word.

-However, we do not use the probability scores obtained from above. Instead we use learned weights as embedding vectors.

---

### A little mathematical background  

I am going to try to break this down as much as possible. The little fragmented parts will help in implementing this
approach later in python.
Alright, let's consider a sample sentence:

> “The sun slowly set behind the mountains, casting a golden glow across the tranquil lake” 

Aim: We are going to train a classifier to find "classify" if given a word 'w' and candidate context word 'c' are to occur together.

So a binary classifier can be thought as a neural network with one output unit which gives the conditional probability that a sample belongs to class A or B.

![Logistic Regression](https://www.researchgate.net/publication/334575492/figure/fig7/AS:782432886349826@1563557945492/Architecture-of-a-Logistic-Regression-Model-56.ppm)


Now for a candidate and word we need to find the below probabilities.
...
$$P({class}=- |w,c) = 1-P({class}=+ |w,c) $$

So the probability for class + means that given candidate word is a real context word for 'w'. (Textbook explanation)
...
I guess based on the corpus we train this classifier on , The probability {P(w,c)} for (golden,glow) will be higher than (golden,lake)
Ofcourse this doesn't seem intuitive because of the senetecne I chose. But the contextual meaning 
